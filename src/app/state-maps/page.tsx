...
const StateMaps = () => {
  const [stateMapData, setStateMapData] = useState<StateMapItem[]>([]);
  ...
  // Fetch state list from server
  useEffect(() => {
    fetch('/api/state-maps')
      .then((res) => res.json())
      .then((data) => setStateMapData(data.states || []))
      .catch(() => setStateMapData([]));
  }, []);
  ...
  const loadMap = async () => {
    ...
    try {
      const response = await fetch(`/api/state-maps?state=${encodeURIComponent(selectedState)}`);
      const data = await response.json();
      if (data.mapUrl) {
        setStateMapUrl(data.mapUrl);
  ...
  const openInNewTab = () => {
    // Map URL is proxied server-side, open the download proxy instead
    window.open(`/api/state-maps/download?state=${encodeURIComponent(selectedState)}`, '_blank', 'noopener,noreferrer');
  };

  const downloadMap = async () => {
    try {
      toast.info('Starting download...');
      const response = await fetch(`/api/state-maps/download?state=${encodeURIComponent(selectedState)}`);
